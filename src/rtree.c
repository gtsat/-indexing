...
void delete_rtree (tree_t *const tree) {
	if (tree != NULL) {
		...
		lseek (fd,0,SEEK_SET);
		write (fd,&tree->dimensions,sizeof(unsigned));
		write (fd,&tree->page_size,sizeof(unsigned));
		write (fd,&tree->tree_size,sizeof(unsigned));
		write (fd,&tree->indexed_records,sizeof(size_t));
		close (fd);
...
static void print_box (boolean stream,tree_t const*const tree, interval_t* box) {
	for (unsigned j=0; j<tree->dimensions; ++j)
		fprintf(stream?stderr:stdout,"(%f,%f) ",box[j].start,box[j].end);
...
static int low_level_write_of_page_to_disk (tree_t *const tree, page_t *const page, unsigned const position) {
	int fd = open (tree->filename, O_WRONLY | O_CREAT, PERMS);
	if (fd < 0) {
		...
		if (lseek (fd,(1+position)*tree->page_size,SEEK_SET) < 0) {
			LOG (error,"Cannot write page at position %u in '%s'...\n",position,tree->filename);
			close (fd);
			exit (EXIT_FAILURE);
		}
		...
		memset (buffer,0,tree->page_size);
		...
		if (page->header.is_leaf) {
			LOG (info,"Flushing leaf-node at position %u with %u records.\n",position,page->header.records);
			...
			LOG (info,"Flushing internal node at position %u with %u children.\n",position,page->header.records);
		...
		unsigned bytelength = ptr - buffer;
		if (bytelength > tree->page_size) {
			LOG (error,"Over-flown page at position %u occupying %u bytes when block-size is %u...\n",position,bytelength,tree->page_size);
			close (fd);
			exit (EXIT_FAILURE);
		}
		if (write (fd,buffer,tree->page_size) != tree->page_size) {
			LOG (error,"Unable to flush page at position %u in '%s'...\n",position,tree->filename);
			close (fd);
			exit (EXIT_FAILURE);
		}
...
static int flush_page (tree_t *const tree, unsigned const page_id) {
...
static int truncate_heapfile (tree_t const*const tree) {
	...
	LOG (info,"Truncating heap-file to %u pages.\n",id+1);
...
/**
			...
			if (entry->key) {
				if (page->header.is_leaf) {
					for (register unsigned i=0; i<page->header.records; ++i) {
...
unsigned anchor (tree_t const*const tree, unsigned id) {
	unsigned sum = 0;
	unsigned product = 1;
	while (sum <= id) {
		sum += product;
...
static boolean update_box (tree_t *const tree, unsigned const page_id) {
	...
	unsigned const offset = CHILD_OFFSET(page_id);
	if (page->header.is_leaf) {
		for (register unsigned i=0; i<page->header.records; ++i) {
			for (unsigned j=0; j<tree->dimensions; ++j) {
...
static void update_boxes (tree_t *const tree, unsigned page_id) {
	for (;page_id
...
size_t compute_page_priority (tree_t *const tree,unsigned const page_id) {
...
page_t* load_rtree_page (tree_t *const tree, unsigned const position) {
	/*
	...
	*/
	...
	if (page_lock != NULL) {
		if (page != NULL) {
			...
			LOG (error,"Inconsistency in page/lock %u...\n",position);
			exit (EXIT_FAILURE);
		}
	}else{
		IO_COUNTER++;
		...
		if (lseek (fd,(1+position)*tree->page_size,SEEK_SET) < 0) {
			LOG (error,"There are less than %u pages in file '%s'...\n",position+1,tree->filename);
			close (fd);
			return page;
		}
		...
		if (page == NULL) {
			LOG (error,"Unable to reserve additional memory to load page %u from the external memory...\n",position);
			close (fd);
			exit (EXIT_FAILURE);
		}
		...
		void *const buffer = (void *const) malloc (tree->page_size), *ptr;
		if (buffer == NULL) {
			LOG (error,"Unable to buffer page %u from the external memory...\n",position);
			abort ();
		}
		if (read (fd,buffer,tree->page_size) < tree->page_size) {
			LOG (warn,"Read less than %u bytes from page %u in '%s'...\n",tree->page_size,position,tree->filename);
		}
		...
		LOG (info,"Loaded from '%s' page %u with %u records from the disk.\n",
								tree->filename,position,page->header.records);

		pthread_rwlock_wrlock (&tree->tree_lock);
		unsigned swapped = set_priority (tree->swap,position,compute_page_priority(tree,position));
		pthread_rwlock_unlock (&tree->tree_lock);
...
fifo_t* transpose_subsumed_pages (tree_t *const tree,
				unsigned const from,
				unsigned const to) {
	...
	while (original->size) {
		...
		unsigned const original_id = (unsigned const) remove_head_of_queue (original);
		unsigned const transposed_id = (unsigned const) remove_head_of_queue (transposed);

		if (original_id == transposed_id) continue;

		LOG (info,"Block at position %u will be transposed to position %u.\n",original_id,transposed_id);
		...
		*/
		...
		page->header.is_dirty = true;
		if (!page->header.is_leaf) {
			for (unsigned offset=0;offset<page->header.records;++offset) {
...
/**
static unsigned transpose_page_position (unsigned page_id,va_list args) {
	tree_t const*const tree = va_arg (args,tree_t const*const);
	unsigned anchorage =  anchor(tree,page_id);
...
static void new_root (tree_t *const tree) {
	...
	if (tree->heapfile_index->size) {
		...
		while (transposed_ids->size) {
			...
			if (true) { //!dump_transposed_pages || entry->key==1) {
				...
				unsigned swapped = set_priority (tree->swap,entry->key,compute_page_priority(tree,entry->key));
				assert (is_active_identifier (tree->swap,entry->key));
				pthread_rwlock_unlock (&tree->tree_lock);
	...
	unsigned swapped = set_priority (tree->swap,0,compute_page_priority(tree,0));
...
tree_t* load_rtree (char const filename[]) {
	...
	if (fd < 0) {
		...
		read (fd,&tree->dimensions,sizeof(unsigned));
		read (fd,&tree->page_size,sizeof(unsigned));
		read (fd,&tree->tree_size,sizeof(unsigned));
		read (fd,&tree->indexed_records,sizeof(size_t));
	}
	...
	LOG (info,"Heap-file in '%s' consists of %u pages and has %lu %u-dimensional records.\n",
...
tree_t* new_rtree (char const filename[], unsigned const page_size, unsigned const dimensions) {
	umask ( S_IRWXO | S_IWGRP);
	tree_t *const tree = (tree_t *const) malloc (sizeof(tree_t));
	if (tree == NULL) {
		LOG (error,"Unable to allocate memory for new R-Tree...\n");
...
static unsigned split_internal (tree_t *const, unsigned, fifo_t *const);

static unsigned halve_internal (tree_t *const tree,
				unsigned position,
				fifo_t *const inception_queue) {

	if (verbose_splits) {
		puts ("==============================================================");
		LOG (info,"HALVING INTERNAL NODE AT POSITION: %u\n",position);
	}
	...
	unsigned parent_records = parent->header.records;
	pthread_rwlock_unlock (parent_lock);
	...
	if (parent_records >= tree->internal_entries) {
		if (position) {
			...
			unsigned const parent_id = split_internal (tree,PARENT_ID(position),inception_queue);
			position = (unsigned) remove_tail_of_queue (inception_queue);
			...
			do{
				unsigned inception = (unsigned) remove_head_of_queue (inception_queue);
	...
	unsigned const lo_offset = CHILD_OFFSET(position);
	unsigned const hi_offset = parent_records;

	unsigned const lo_id = position;
	unsigned const hi_id = CHILD_ID(PARENT_ID(position),hi_offset);
	...
	for (unsigned j=0; j<tree->dimensions; ++j) {
		for (register unsigned i=0; i<overloaded_page->header.records; ++i) {
	...
	unsigned old_child=0, new_child=0, new_id=0;
	insert_at_head_of_queue (inception_queue,UINT_MAX);
	unsigned inception = (unsigned) remove_tail_of_queue (inception_queue);

	fifo_t* transposed_ids = new_queue();

	assert (lo_page->header.records == lo_pages->size);
	for (unsigned i=lo_page->header.records-1; lo_pages->size; --i) {
		unsigned const offset = (unsigned const) remove_from_stack (lo_pages);
		...
		if (page != NULL) {
			...
			LOG (info,"Node with id %u (compared against %u) is now under %u with new id %u.\n",old_child,inception,lo_id,new_id);
			if (!update_flag && inception == old_child) {
				insert_at_head_of_queue (inception_queue,new_id);
				do{
					inception = (unsigned) remove_tail_of_queue (inception_queue);
					if (inception != UINT_MAX) {
			...
			unsigned new_size = transposed_ids->size + tmp_queue->size;
			...
			LOG (error,"Unable to retrieve heap-file entry #%u...\n",CHILD_ID(position,offset));
			exit (EXIT_FAILURE);
		}
	}

	unsigned const new_position = update_flag ? position : hi_id;

	assert (hi_page->header.records == hi_pages->size);
	for (unsigned i=hi_page->header.records-1; hi_pages->size; --i) {
		unsigned const offset = (unsigned const) remove_from_stack (hi_pages);
		page_t *const page = load_rtree_page (tree,CHILD_ID(position,offset));
		if (page != NULL) {
	...
	LOG (info,"Node with id %u is now under %u.\n",lo_id,PARENT_ID(lo_id));
	LOG (info,"Node with id %u is now under %u.\n",hi_id,PARENT_ID(hi_id));
	LOG (info,"Halved internal node with new identifier %u and sibling new node "
			"%u having %u records into two parts of %u and %u records.\n",
	...
	for (register unsigned i=0; i<lo_page->header.records; ++i) {
		if (i) {
			for (unsigned j=0; j<tree->dimensions; ++j) {
	...
	for (register unsigned i=0; i<hi_page->header.records; ++i) {
		if (i) {
			for (unsigned j=0; j<tree->dimensions; ++j) {
...
/*
	if (dump_transposed_pages) {
		unsigned const parent_id = PARENT_ID(new_position);
...
*/
	LOG (info,"DONE HALVING INTERNAL NODE AT POSITION %u WITH NEW ID %u.\n",position,new_position);
	delete_internal (overloaded_page);
	return new_position;
}

static unsigned split_internal (tree_t *const tree,
				unsigned position,
				fifo_t *const inception_queue) {
	if (verbose_splits) {
		puts ("==============================================================");
		LOG (info,"SPLITTING INTERNAL NODE AT POSITION: %u\n",position);
	}
	...
	if (parent_records >= tree->internal_entries) {
		if (position) {
			...
			do{
				unsigned inception = (unsigned) remove_head_of_queue (inception_queue);
				if (inception != UINT_MAX) {
					unsigned transposed_inception = TRANSPOSE_PAGE_POSITION(inception);
	...
	for (register unsigned j=0; j<tree->dimensions; ++j) {
		for (register unsigned i=0; i<overloaded_page->header.records; ++i) {
			box_container_t *const box_container_lo = (box_container_t *const) malloc (sizeof(box_container_t));
			box_container_t *const box_container_hi = (box_container_t *const) malloc (sizeof(box_container_t));
			if (box_container_lo == NULL || box_container_hi == NULL) {
				LOG (error,"Unable to reserve additional memory to split page %u...\n",position);
				exit (EXIT_FAILURE);
			}
		...
		unsigned lo_records = 0;
		unsigned hi_records = overloaded_page->header.records;

		for (unsigned insertions=0,deletions=0; priority_queue->size;) {
			...
			if (box_container->sort_key == box_container->box[j].start) {
				...
				LOG (error,"Invalid dimension %u interval encountered (%f,%f)...\n",
					j,box_container->box[j].start,box_container->box[j].end);
				exit (EXIT_FAILURE);
			}
	...
	LOG (info,"Selected split-zone is (%f,%f) along dimension %u achieving fairness: %f.\n",
											splitzone.start,splitzone.end,splitdim,fairness);
	...
	if (fairness >= fairness_threshold) {
		...
		unsigned const hi_id = CHILD_ID(PARENT_ID(position),parent_records);
		unsigned old_child = 0, new_child = 0, new_id = 0;
		...
		for (unsigned i=0; i<overloaded_page->header.records; ++i) {
			...
			if (box_ptr[splitdim].end <= splitzone.start) {
				...
				LOG (info,"Node with id %u is now under %u with new id %u.\n",old_child,lo_id,new_id);
				if (!update_flag && inception == old_child) {
					insert_at_head_of_queue (inception_queue,new_id);
					do{
						inception = (unsigned) remove_tail_of_queue (inception_queue);
				...
				LOG (error,"Unclaimed page #%u while moving it from page #%u...\n",old_child,position);
				exit (EXIT_FAILURE);
			}
		}
		...
		LOG (info,"Node with id %u is now under %u.\n",lo_id,PARENT_ID(lo_id));
		LOG (info,"Node with id %u is now under %u.\n",hi_id,PARENT_ID(hi_id));
		LOG (info,"Split by dimension %u leaf node with new identifier %u and sibling new node "
				"%u having %u records into two parts of %u and %u records.\n",
		...
		for (register unsigned i=1; i<lo_page->header.records; ++i) {
			for (unsigned j=0; j<tree->dimensions; ++j) {
		...
		for (register unsigned i=1; i<hi_page->header.records; ++i) {
			for (unsigned j=0; j<tree->dimensions; ++j) {
...
*/
		LOG (info,"DONE SPLITTING INTERNAL NODE AT POSITION %u WITH NEW ID %u.\n",position,new_position);
		delete_internal (overloaded_page);
		return new_position;
	}else{
...
static unsigned split_leaf (tree_t *const tree, unsigned position) {
	LOG (info,"SPLITTING LEAF NODE AT POSITION: %u\n",position);
	...
	unsigned const hi_offset = parent->header.records;
	pthread_rwlock_unlock (parent_lock);
	...
	for (unsigned j=1; j<tree->dimensions; ++j) {
	...
	pthread_rwlock_wrlock (parent_lock);
	for (register unsigned i=0; i<(overloaded_page->header.records>>1); ++i) {
...
static void cascade_deletion (tree_t *const tree,
								unsigned const page_id,
								unsigned const offset) {

	LOG(info,"CASCADED DELETION TO PAGE %u.\n",page_id);
	...
	if (page->header.records >= fairness_threshold*(tree->internal_entries>>1)
		...
		unsigned const deleted_page_id = CHILD_ID(page_id,offset);
		unsigned const replacement_page_id = CHILD_ID(page_id,page->header.records-1);
		...
		while (browse->size) {
			unsigned const subsumed_id = (unsigned const) remove_from_stack (browse);
			...
			if (subsumed_page->header.is_leaf) {
				for (register unsigned i=0; i<subsumed_page->header.records; ++i) {
		...
		LOG(error,"Erroneous cascaded deletion to remove page %u enacted from page %u...",
...
object_t delete_from_rtree (tree_t *const tree, index_t const key[]) {
	...
	while (browse->size) {
		unsigned const page_id = (unsigned const) remove_from_stack (browse);
		...
		if (page->header.is_leaf) {
			for (unsigned i=0; i<page->header.records; ++i) {
				if (equal_keys (page->node.leaf.keys+i*tree->dimensions,key,tree->dimensions)) {
					...
					if (page_id && page->header.records < fairness_threshold*(tree->leaf_entries>>1)) {
						...
						for (unsigned j=0; j<page->header.records; ++j) {
							if (j!=i) {
								pthread_rwlock_wrlock (&tree->tree_lock);
								tree->indexed_records--;
								pthread_rwlock_unlock (&tree->tree_lock);
...
static void process_records_from_textfile (tree_t *const tree, char const filename[], boolean const insert) {
	...
	if (fptr == NULL) {
		...
		while (!feof(fptr)) {
			fscanf (fptr,"%u ",&id);
			if (insert) {
				LOG(info,"Indexing value '%u' by key: (",id);
			}else{
				LOG(info,"Deleting value '%u' indexed by key: (",id);
			}
			...
			for (unsigned i=0; i<tree->dimensions; ++i) {
				...
				if (logging <= info) {
					fprintf (stderr,"%f ",coordinates[i]);
				}
			}
...
static index_t expansion_volume (index_t const key[], interval_t const box[], unsigned const dimensions) {
	index_t volume = 0;
...
static int compare_expansion_dummy (void const*const xcontainer, void const*const ycontainer) {
	...
	unsigned const x = ((dummy_t*)xcontainer)->page_id;
	unsigned const y = ((dummy_t*)ycontainer)->page_id;
...
void insert_into_rtree (tree_t *const tree, index_t const key[], object_t const value) {
	...
	while (browse->size) {
		unsigned const position = (unsigned const) remove_from_stack (browse);
	...
	if (minleaf != NULL) {
		...
		unsigned minleaf_records = minleaf->header.records;
		...
		if (minleaf_records >= tree->leaf_entries) {
			...
			unsigned const sibling = CHILD_ID(PARENT_ID(minpos),load_rtree_page (tree,PARENT_ID(minpos))->header.records-1);
		...
		while (!is_inserted && volume_expansion_priority_queue->size) {
			...
			unsigned position = container->page_id;
			...
			if (page->header.is_leaf) {
				if (page->header.records >= tree->leaf_entries) {
					...
					unsigned const sibling = CHILD_ID(PARENT_ID(position),load_rtree_page (tree,PARENT_ID(position))->header.records-1);
				...
				for (;position;position=PARENT_ID(position)) {
					...
					unsigned const offset = CHILD_OFFSET(position);
