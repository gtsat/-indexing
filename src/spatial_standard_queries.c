...
fifo_t* bounded_search (tree_t *const tree,
		...
		index_t const center[], unsigned const k,
...
fifo_t* nearest (tree_t *const tree, index_t const query[], unsigned const k) {
...
fifo_t* multichromatic_reverse_nearest_neighbors (index_t const query[],
	...
	for (unsigned itree=0; itree<feature_trees->size; ++itree) {
			...
			if (pthread_rwlock_tryrdlock (page_lock)) {
				...
				if (page->header.is_leaf) {
					for (register unsigned i=0; i<page->header.records; ++i) {
						...
						for (unsigned c=0;c<=itree;++c) {
							for (unsigned j=0; j<cells[c]->size; ++j) {
					...
					unsigned i = page->header.records-1;
					do{
						container = (box_container_t*) malloc (sizeof(box_container_t));
		...
		if (pthread_rwlock_tryrdlock (page_lock)) {
			...
			if (page->header.is_leaf) {
				for (register unsigned i=0; i<page->header.records; ++i) {
					...
					boolean is_obscured_data_item = false;
					for (unsigned c=0;c<feature_trees->size;++c) {
						for (unsigned j=0; j<cells[c]->size; ++j) {
							data_container_t* adjacent_cell_center = (data_container_t*) cells[c]->buffer[j];
	...
	for (unsigned i=0; i<feature_trees->size; ++i) {
...
/*
 * Opt between joining over the distance average,
 * or the maximum distance in the combination.
 */

fifo_t* distance_join0 (double theta,
	...
	if (less_than_theta && theta < 0) {
		LOG (warn,"No point for negative distances for predicate joins...\n");
		return new_queue();
	}

	/*
	 * Because implemented distance functions return squares.
	 * No need to waste CPU cycles for computing the square root!!
	 */
	theta *= theta;
	...
	while (browse->size) {
		...
		for (unsigned i=0; i<container->cardinality; ++i) {
			...
			if (pthread_rwlock_tryrdlock (page_lock)) {
				...
				if (!page->header.is_leaf) {
					...
					unsigned j = page->header.records-1;
					do{
		...
		if (all_leaves) {
			insert_into_stack (leaves,container);
		}else{
	...
	LOG (info,"Taking place spatial join of %u leaf-page combination(s) from %u trees.\n",leaves->size,cardinality);

	priority_queue_t* data_combinations = new_priority_queue (less_than_theta?&mincompare_multicontainers:&maxcompare_multicontainers);

	while (leaves->size) {
		container = remove_from_stack (leaves);
		...
		for (unsigned i=0; i<cardinality; ++i) {
			...
			if (pthread_rwlock_tryrdlock (page_lock)) {
				while (leaves->size) {
					multibox_container_t* temp = remove_from_stack (leaves);
		...
		for (unsigned j=0,i=0;;++j) {
			if (offsets[i] >= pages[i]->header.records) {
				...
				for (unsigned offset=0; offset<cardinality; ++offset) {
...
fifo_t* closest_tuples_ordered  (unsigned const k,
				boolean const use_avg,
				tree_t *const tree0,...) {
	va_list args;
	va_start (args,tree0);
...
fifo_t* farthest_tuples_ordered (unsigned const k,
				boolean const use_avg,
				tree_t *const tree0,...) {
	va_list args;
	va_start (args,tree0);
...
fifo_t* closest_tuples_pairwise  (unsigned const k,
				boolean const use_avg,
				tree_t *const tree0,...) {
	va_list args;
	va_start (args,tree0);
...
fifo_t* farthest_tuples_pairwise (unsigned const k,
				boolean const use_avg,
				tree_t *const tree0,...) {
	va_list args;
	va_start (args,tree0);
